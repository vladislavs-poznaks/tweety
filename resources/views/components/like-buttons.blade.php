<div class="flex items-center space-x-5 text-xs mt-3">
    <form action="{{ route('tweet.like', $tweet) }}" method="POST">
        @csrf
        <div
            class="px-2 py-1 rounded-full
                            {{ $tweet->isLikedBy(auth()->user()) ? 'bg-blue-200' : 'bg-gray-200' }}
    ...
    <form action="{{ route('tweet.dislike', $tweet) }}" method="POST">
        ...
        <div
            class="px-2 py-1 rounded-full
                            {{ $tweet->isDislikedBy(auth()->user()) ? 'bg-blue-200' : 'bg-gray-200' }}
            {{ $tweet->isDislikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
            <button type="submit" class="flex items-stretch space-x-1">
                <svg viewBox="0 0 20 20" class="w-3">
                    <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                        <g class="fill-current">
                            <path
